...
from model.components import EncoderDINO, ProjectionHead
...
class SAC_DINO:
    def __init__(self, env, hps, train_hps, device):
        ...
        self.crop = Crop(self.global_crop_size, self.local_crop_size)

        self.replay_buffer = ReplayBuffer(
            self.buffer_capacity, self.obs_shape, self.action_shape, device
        )

        self.encoder = EncoderDINO(self.obs_shape, hps.embed_dim, hps.encoder)
        self.projection = ProjectionHead(hps.embed_dim, hps.projection)

        self.target_encoder = EncoderDINO(self.obs_shape, hps.embed_dim, hps.encoder)
    ...
    def _init_hyperparameters(self):
        ...
        self.lambda_dino = float(self.loss_hps.lambda_dino)
    ...
    def dino_loss(self, global_views, local_views):
        B, N_global, C, H_global, W_global = global_views.shape
        _, N_local, _, H_local, W_local = local_views.shape

        target_temp = self._target_temp_schedule(self.num_updates)
        ...
        loss = loss.mean() * self.lambda_dino

        return loss, logits, target_logits

    def update_parameters(self):
        batch = self.replay_buffer.sample(self.batch_size)
        obs, action, reward, n_obs, done = batch
        ...
        for _ in range(self.num_updates_per_batch):
            self.num_updates += 1
            global_views, local_views = self.crop(obs, self.num_global, self.num_local)
            obs_q = global_views[:, 0]
            next_obs = self.crop.single_crop(n_obs, self.crop.global_crop)
            ...
            dino_loss, logits, target_logits = self.dino_loss(global_views, local_views)
            ...
            with torch.no_grad():
                ...
                if self.num_updates % self.center_update_freq == 0:
        ...
        while total_timesteps < self.total_timesteps:
            ...
            while not done:
                ...
                if self.replay_buffer.size > self.start_training_steps:
                    ...
                    if self.total_timesteps % self.checkpoints_freq == 0:
                        torch.save(
                            {
