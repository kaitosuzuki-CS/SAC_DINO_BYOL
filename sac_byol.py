...
from model.components import EncoderBYOL
...
class SAC_BYOL:
    def __init__(self, env, hps, train_hps, device):
        ...
        self.obs_shape, self.crop_shape, self.action_spec, self.action_shape = (
            self._get_env_info()
        )

        self.crop = Crop(self.global_crop_size)

        self.replay_buffer = ReplayBuffer(
            self.buffer_capacity, self.obs_shape, self.action_shape, device
        )

        self.encoder = EncoderBYOL(self.crop_shape, hps.embed_dim, hps.encoder)
        self.projection = nn.Linear(hps.encoder.output_dim, hps.encoder.output_dim)

        self.target_encoder = EncoderBYOL(self.crop_shape, hps.embed_dim, hps.encoder)
        ...
        self.target_critic1 = SoftCritic(
            self.target_encoder, self.action_shape[0], hps.embed_dim, hps.critic
        )
    ...
    def _init_hyperparameters(self):
        ...
        self.num_updates_per_batch = int(self._train_hps.num_updates_per_batch)
    ...
    def _get_env_info(self):
        ...
        crop_shape = self._hps.crop_shape
        action_spec = self._env.action_spec()
        action_shape = action_spec.shape

        return obs_shape, crop_shape, action_spec, action_shape
    ...
    def byol_loss(self, q, k):
        ...
        with torch.no_grad():
            z_k = self.target_encoder(k).detach()

        z_q = F.normalize(z_q, dim=-1)
        z_k = F.normalize(z_k, dim=-1)

        loss = F.mse_loss(z_q, z_k)
    ...
    def update_parameters(self):
        ...
        for _ in range(self.num_updates_per_batch):
            self.num_updates += 1
            obs_q, obs_k = self.crop.single_crop(
                obs, self.crop.global_crop
            ), self.crop.single_crop(obs, self.crop.global_crop)
            ...
            byol_loss1 = self.byol_loss(obs_q, obs_k)
            byol_loss2 = self.byol_loss(obs_k, obs_q)
            byol_loss = byol_loss1 + byol_loss2
            ...
            pi, log_pi, _, _ = self.actor(obs_q)
            ...
            with torch.no_grad():
                if self.num_updates % self.target_update_freq == 0:
                    m = self._target_momentum_schedule(self.num_updates)
        ...
        while total_timesteps < self.total_timesteps:
            ...
            while not done:
                ...
                if self.replay_buffer.size > self.start_training_steps:
                    if total_timesteps % self.update_freq == 0:
                        self.update_parameters()

                    if total_timesteps % self.checkpoints_freq == 0:
                        torch.save(
                            {
                            ...
                            f"{save_dir}/checkpoints_{self.num_updates}.pt",
                        )
